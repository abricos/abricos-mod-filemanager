...
Component.entryPoint = function(){
    ...
    YAHOO.extend(ImageEditorPanel, Brick.widget.Dialog, {
        ...
        selectTools: function(tools){
            ...
            switch (tools) {
                case 'size':
                    this.selectToolsSize();
                    break;
                case 'crop':
                    this.selectToolsCrop();
                    break;
            }
        },
        ...
        destroyTools: function(){
            switch (this.selectedTools) {
                case 'crop':
                    this.toolsCrop.destroy();
                    break;
                case 'size':
                    this.toolsSize.destroy();
                    break;
            }
            this.selectTools('');
        },
        onClick: function(el){
            ...
            switch (el.id) {
                case tp['btapply']:
                    this.applyToolsChanges();
                    return true;
                case tp['btcancel']:
                    this.cancelToolsChanges();
                    return true;
                case tp['btsize']:
                    this.selectTools('size');
                    return true;
                case tp['btcrop']:
                    this.selectTools('crop');
                    return true;
                case tp['bsave']:
                    this.save();
                    return true;
